...
function applyFirewallRules {
  ...
  if [[ $(firewall-cmd --zone=trusted --list-sources | grep -c "$kira_worker1") == 0 ]]; then
    logit "applyFirewallRules: Adding source $kira_worker1 to trusted zone"
    /usr/bin/firewall-cmd --permanent --zone=trusted --add-source=$kira_worker1
  fi 

  if [[ $(firewall-cmd --zone=trusted --list-sources | grep -c "$kira_worker2") == 0 ]]; then
    logit "applyFirewallRules: Adding source $kira_worker2 to trusted zone"
    /usr/bin/firewall-cmd --permanent --zone=trusted --add-source=$kira_worker2
  fi 
